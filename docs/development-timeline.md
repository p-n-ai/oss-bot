...
| `B-W6D26-5` | `internal/validator/resolver.go` — `SchemaResolver` with two-tier schema resolution: per-subject override (`{subjectID}/schema/`) + global fallback (`schema/`). `FindSubjectDir()` walks up from any YAML file to find the subject directory. `SubjectSchemaDir()` checks for `schema/` subfolder. Resolution is per-schema-file (mix-and-match). | 🤖 | ✅ | TDD: 10 resolver tests passing |
...
| `B-W6D26-7` | `oss scaffold subject` copies 6 global schema files into `{subjectID}/schema/` for per-subject customization. Added `GlobalSchemaDir` to `ScaffoldRequest`. Silently skips if no global schemas exist. | 🤖 | ✅ | TDD: 2 new scaffolder tests |
